                    ...
                    public void onComplete(Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            if (task.getException().getClass().getName().equals(exceptionString)) {
                                Toast.makeText(SignUpActivity.this, "Email adresinizin formatı kötü",
                                        Toast.LENGTH_SHORT).show();
                                updateUI(null);
                            }

                            else if (task.getException().getClass().getName().equals(exceptionEmailCollisionString)) {
                                Toast.makeText(SignUpActivity.this, "Bu epostaya ait kullanıcı mevcut",
                                        Toast.LENGTH_SHORT).show();
                                updateUI(null);
                            }

                            else {
                                Toast.makeText(SignUpActivity.this, "Kimlik doğrulama başarısız.",
                                        Toast.LENGTH_SHORT).show();
                                updateUI(null);
                            }
                        }
    ...
    private boolean validateForm() {
        boolean valid = true;
            ...
            if (TextUtils.isEmpty(email)) {
                mEmailSignUpField.setError("Gerekli.");
                valid = false;
            } else {
            ...
            if (TextUtils.isEmpty(password)) {
                mPasswordSignUpField.setError("Gerekli.");
                valid = false;
            } else {
